...
describe('buildOutline', () => {
  it('includes only heading rows, in order, labeled by their translation', () => {
    const rows = [
      row('Articulus 1', { headingLevel: 1, english: buildRowDoc([t('Article 1')]).toJSON() }),
      row('Ad primum sic proceditur'),
      row('Utrum Deus sit', { headingLevel: 3, english: buildRowDoc([t('Whether God exists')]).toJSON() }),
    ];
    expect(buildOutline(rows)).toEqual([
      { rowIndex: 0, level: 1, label: 'Article 1' },
      { rowIndex: 2, level: 3, label: 'Whether God exists' },
    ]);
  });
